...
# # 🔥 MOVE DATA TO GPU ONCE (ADD HERE)
# spectral_data = torch.tensor(spectral_data).float().unsqueeze(1).to(DEVICE)
# window_regimes = torch.tensor(window_regimes).long().to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    num_batches = 0
    indices     = np.random.permutation(len(spectral_data))
    # indices = torch.randperm(len(spectral_data), device=DEVICE)
    ...
    for i in tqdm(range(0, len(indices), BATCH_SIZE), desc=f"Epoch {epoch+1}/{EPOCHS}"):
        ...
        # x = spectral_data[batch_idx]        # already on GPU
        # r = window_regimes[batch_idx]      # already on GPU
        t = torch.randint(0, T, (x.size(0),), device=DEVICE)
